...
2. **论文标题**: 识别第一页最大字号文本
3. **作者**: 识别署名,最多列出前3位,格式如 "张三, 李四, 王五 et al."
4. **器件结构**: 识别层级堆叠结构,如 ITO/PEDOT:PSS/EML/ZnO/Al
5. **实验参数**: 
   - EQE(外量子效率): 提取所有器件的数值,多个数值以换行符分隔
   - CIE(色度坐标): 提取所有器件的坐标,格式如 (0.21, 0.32),多个以换行符分隔
   - 寿命(T50/LT50): 提取所有器件的寿命数值和单位,多个以换行符分隔
6. **数据溯源**: 提取数值时,附带原文出处句子
7. **优化层级和策略**: 总结论文中的优化方法,包括优化层级和具体策略,约100字
...
1. 如果某字段无法从文本中提取,请设为空字符串 "" 或空数组 []
2. 多个器件的参数值必须分别列出,不要合并
3. 数据溯源字段要包含原文中的完整句子
4. 优化策略描述要简洁,控制在100字左右
