...
1. 仅改写自然语言文本字段，输出格式固定为：`中文 | English`
2. 如果原文是英文，输出：`中文翻译 | Original English`
3. 如果原文是中文，输出：`原始中文 | English translation`
4. 如果字段已经是 `中文 | English` 形式，保持原意并可微调措辞，但不要重复翻译
...
2. 保持 JSON 可被直接解析
3. 不要把结构化数据改写成长段落
4. 对 data_source 中的原文引用，保留原句含义，输出中英对照
