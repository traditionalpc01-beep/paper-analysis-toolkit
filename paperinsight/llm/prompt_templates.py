...
LITE_PAPER_INFO_BACKFILL_PROMPT = """?????????????????????
...
def format_lite_paper_info_backfill_prompt(
    ...
    metadata_json = json.dumps(metadata or {}, indent=2, ensure_ascii=False)
    return LITE_PAPER_INFO_BACKFILL_PROMPT.format(
        paper_text=paper_text,
